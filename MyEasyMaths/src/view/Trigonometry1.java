 ...
 */
public class Trigonometry1 extends PanelWithBackground implements Observer {
	...
	public Trigonometry1(Model model, Controller controller) {
		...
		add(new MessageLabel(
				"<html>Give an input of 2 angles and 1 side that is not<br>inbetween the 2 angles and I will solve the triangle:<br><br>"));
		...
		try {
			picture = ImageIO.read(new File("src/images/triangle1.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void update(Observable o, Object arg) {
		if (model.getCurrentPanelIndex() == 11) {

			String angleC = model.angles_add_up_to_180(A.getText(), B.getText());
			String side_a = model.sine_law_to_find_side(A.getText(), B.getText(), b.getText());
			String side_c = model.sine_law_to_find_side(angleC, B.getText(), b.getText());

			String DegreeSign = "\u00b0";

			a_answer.setText(side_a);
			b_answer.setText(b.getText());
			c_answer.setText(side_c);
			A_answer.setText(A.getText() + DegreeSign);
			B_answer.setText(B.getText() + DegreeSign);
			C_answer.setText(angleC + DegreeSign);

			String area = model.getTriangleArea(side_c, b.getText(), A.getText());
